...
parser.add_argument(
    "--nav_ros_map_odom_rotation",
    type=float,
    nargs=4,
    default=None,
    metavar=("X", "Y", "Z", "W"),
    help="override the map->odom rotation as a ROS xyzw quaternion; defaults to the robot chassis world orientation at bridge creation",
)
parser.add_argument("--enable_nav_ros_pointcloud", action="store_true", default=False, help="enable ROS2 PointCloud2 bridge for the G1 front RGBD camera")
...
parser.add_argument("--enable_nav_ros_rgbd_images", action="store_true", default=False, help="enable ROS2 RGB/depth image bridge for the G1 front RGBD camera")
...
parser.add_argument("--nav_ros_pointcloud_topic", type=str, default="/g1/head_rgbd/points", help="ROS2 PointCloud2 topic for the G1 front RGBD camera")
parser.add_argument("--nav_ros_rgb_topic", type=str, default="/g1/head_rgbd/rgb/image_raw", help="ROS2 RGB image topic for the G1 front RGBD camera")
parser.add_argument("--nav_ros_depth_topic", type=str, default="/g1/head_rgbd/depth/image_raw", help="ROS2 depth image topic for the G1 front RGBD camera")
parser.add_argument("--nav_ros_camera_info_topic", type=str, default="/g1/head_rgbd/camera_info", help="ROS2 CameraInfo topic for the G1 front RGBD camera")
parser.add_argument("--nav_ros_camera_frame", type=str, default="g1_front_rgbd_optical_frame", help="ROS2 frame id for the G1 front RGBD camera")
...
parser.add_argument("--nav_ros_camera_width", type=int, default=640, help="render product width for the G1 front RGBD camera")
parser.add_argument("--nav_ros_camera_height", type=int, default=480, help="render product height for the G1 front RGBD camera")
...
parser.add_argument("--nav_action_render_interval", type=int, default=None, help="manual env.sim.render interval for nav_minimal_dds GUI runs; defaults to every provider tick, ignored by pointcloud mode")
        ...
        try:
            ...
            bridge_info = create_g1_nav_tf_odom_graph(
                ...
                G1NavTfOdomBridgeConfig(
                    ...
                    map_odom_rotation=args_cli.nav_ros_map_odom_rotation,
                ),
            )
